...
def edit_profile(request):
    ...
    if us_id == None:
        ...
        result = User_profile.objects.get(user_id=us_id)
...
def supplier_forget_chk(request):
    ...
    if supplier is None:
        ...
        send_forget_password_mail_to_supplier(email, branch_name, branch_owner, token)
...
def user_forget(request):
    context={}
    return render(request,'user/user_forget.html',context)


def user_forget_chk(request):
    email = request.POST['email']
    ...
    if id is None:
        ...
        token = supplier_token_generator(user)
        send_forget_password_mail_to_agency(email, fname, lname, token)
        #messages.success(request, 'Check Your Mail ')
        return HttpResponse('Email Send :')
...
def changePassword2(request):
    ...
    if token:
        ...
        return HttpResponseBadRequest('Token is required')
    context = {}
    return render(request, 'user/changePassword2.html', context)
...
def single_product(request,id):
    result = SoftDrink.objects.filter(category_id=id)
    context = {'result':result}
...
def view_my_order(request):
    ...
    if us_id == None:
        ...
        orders = Order.objects.filter(user_id = us_id)
        context = {'orders':orders}
        return render(request,'user/view_my_order.html',context)
...
def track_order(request,id):
    ...
    track=result.track
    context={'track':track,'result':result,'product':product_info,'total':total_cost}
    return render(request,"user/order_track.html",context)

def test(request):
    track=4
    uncheck = 4-track
    context = {'track':track,'uncheck':uncheck}
    return render(request,'user/test.html',context)

def receipt_viewmore(request,id):
    result = Order.objects.get(pk=id)
    product = Product.objects.get(pk = result.product_id)
    cost = result.quantity*product.price
    user_info = User.objects.get(pk=result.user_id)
    ...
    context = {'result':result,'product':product,'cost':cost,'user_info':user_info,'user_profile':user_info,'user_profile':user_profile,'area':area,'city':city}
    return render(request,"user/viewmore.html",context)
#---------------------------------------------------------------
def logout(request):
    auth.logout(request)
    return redirect('/user/login')   
...
def category(request):
    categories = Product_Category.objects.values_list('category_name', flat=True)
    cat = Product_Category.objects.all()
    result1 = Product.objects.all()
    data1 = categories[0].upper()
    data2 = categories[1].upper()
    data3 = categories[2].upper()
    data4 = categories[3].upper()
    data5 = categories[4].upper()
